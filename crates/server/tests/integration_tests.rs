...
mod fixtures;
use fixtures::{dummy_pool, json_body, test_router, test_router_with_rate_limit};
...
async fn demo_crud_happy_path() {
    // TODO: implement full create → get → update → delete cycle once
    // a test-user session can be injected.  Tracked in Phase 1A.
}

#[tokio::test]
#[ignore = "requires DATABASE_URL"]
async fn publish_makes_demo_publicly_accessible() {
    // TODO: implement publish → public access check once session injection
    // is available.  Tracked in Phase 1A.
}
