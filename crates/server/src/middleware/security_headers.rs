...
pub async fn security_headers_middleware(req: Request<Body>, next: Next) -> Response {
    ...
    if is_embed_route {
        headers.insert(
            ...
            HeaderValue::from_static("default-src 'self'; frame-ancestors *; script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' data: https://fonts.gstatic.com"),
        );
    } else {
        headers.insert(
            "content-security-policy",
